...
import type { InvestigationResult, ConversationMessage } from '../types'
import { sendMessageWithTools, continueWithToolResults, type ToolDefinition, type MessageResult } from '../conversation'
...
Answer the following investigation query by exploring the codebase.`
	...
	try {
		...
		while (iterations < maxIterations) {
			...
			const toolUses = results.filter((r): r is { type: 'tool_use'; id: string; name: string; input: Record<string, unknown> } => r.type === 'tool_use')

			if (toolUses.length === 0) {
				// No more tool calls, extract final text
				const textResults = results.filter((r): r is { type: 'text'; content: string } => r.type === 'text')
