...
A standalone Next.js application focused on visualization, review, and merge decisions. It does not control execution — that's Claude Code's job. The dashboard is a read-heavy command center that pulls state from GitHub Projects and execution data from SQLite.

---

## Overview

The dashboard presents a consistent UX regardless of project. Whether you're running one project or five, the views are the same: Phase Overview, Dependency Graph, Merge Report, Execution Tracker. Projects are filterable. When you start a new project, its phases appear in the same interface with the same interactions.
...
| Data Sources | GitHub Projects API (task state, PR data, Greptile reviews) + SQLite (execution tracking) |
| Storage | SQLite via better-sqlite3 — execution tracking and run history (the only data the dashboard owns) |
...
- Color-coded nodes by status:
  ...
  - Done: checkmark
  - Failed: red
- Click a node to see: task details, PR link, Greptile score, agent logs, model used, loop count
...
- Per-task breakdown: task name, model used, loop count, duration
- Per-phase summary: task count, model distribution
- Running total across all phases
- Trend chart: loops per run over time
- Stall alerts: configurable threshold (e.g., warn if a single task exceeds 5 loops)
- Data source: SQLite (execution history that GitHub Projects doesn't track)
...
| `GET /api/execution/summary` | Execution summary (per-task, per-phase, total) |
| `GET /api/execution/history` | Historical execution data for charts |
